...
var ObjectAction = function () {
    ...
    this.addPoint = function (name) {
        ...
        var material = new THREE.PointsMaterial({ size: 1, depthWrite : false, depthTest : false });
    ...
    this.addPoints = function (name) {
        var geometry = new THREE.BufferGeometry();
        var material = new THREE.PointsMaterial({ size: 0.1, depthWrite : false});
        var mesh = new THREE.Points(geometry, material);
        mesh.name = name;
        return mesh;
    }
    ...
    this.addLines = function (name, count = 30) {
	    ...
	    this.addLines2 = function (name, count = 30) {
        var group = new THREE.Group();
        group.name = name;
        for (var i = 0; i < count; i++) {
            var geometry = new THREE.BufferGeometry();
    ...
    this.updateLines = function (lines, data) {
        ...
        for (var i = 0; i < data.numberLines; i++) {
            ...
            lines.children[i].visible = true;
			// lines.children[i].geometry.material.side = THREE.DoubleSide;
			lines.children[i].frustumCulled = false;
			// lines.children[i].material.depthWrite = true;
        }
    }
    ...
    this.updatePolygons = function (polygons, data) {
        ...
        for (var i = 0; i < data.numberPolygons; i++) {
			...
			polygons.children[i].frustumCulled = false;
        }
    }
    ...
    this.updatePointCloud = function (points, data, showIntensity) {
        // console.log("updatePointCloud", points);
        var cloud = [];
        var RGB = [];
        ...
        // points.material.vertexColors = showIntensity?THREE.VertexColors:0;
    ...
    this.updatePointsMaterial = function (mesh, color, size) {
        ...
        // mesh.renderOrder = 2; // z-index值，按照需要设置
    }
    ...
    this.updatePoseMaterial = function (mesh, color, size) {
        ...
        mesh.setLength(size);
		mesh.material.depthWrite = false;
// mesh.renderOrder = 4; // z-index值，按照需要设置
        // setLength (length, headLength, headWidth)
        // length -- The desired length
        // headLength -- The length of the head of the arrow
        // headWidth -- The length of the width of the arrow
